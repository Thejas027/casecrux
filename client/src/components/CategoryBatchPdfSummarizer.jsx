...
function CategoryBatchPdfSummarizer({ onSummaryUpdate, onTranslationUpdate }) {
  ...
  const handleInstantSummarize = async (e) => {
    ...
    try {
      ...
      // Check if this is a demo/fallback response
      const isDemoMode = response.data.overall_summary?.demo_mode ||
        response.data.message?.includes("Fallback response") ||
        response.data.message?.includes("Demo response") ||
        response.data.demo_info;

      if (isDemoMode) {
        // Show informative demo mode notification
        const demoNotification = document.createElement('div');
        demoNotification.className = 'fixed top-4 right-4 bg-blue-600 text-white px-4 py-3 rounded-lg shadow-lg z-50 max-w-sm';
        demoNotification.innerHTML = `
          <div class="flex items-start">
            ...
            <div class="ml-3">
              ...
              <p class="text-xs text-blue-200 mt-1">Showing sample legal analysis - configure GROQ API keys for full AI functionality</p>
            </div>
            <button class="ml-4 text-blue-300 hover:text-white" onclick="this.parentElement.parentElement.remove()">×</button>
          </div>
        `;
  ...
  const handleAdvancedSummarization = async (urls, useComparison = false) => {
    try {
      ...
      const summaryResponse = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/ml/summarize_from_urls`,
        {
          urls: urls,
        ...
        <div className="bg-[#23272f] shadow-2xl rounded-xl px-10 pt-8 pb-10 mb-8 border-2 border-[#7f5af0]">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <button
              ...
              className={`bg-gradient-to-r from-[#2cb67d] to-[#7f5af0] hover:from-[#7f5af0] hover:to-[#2cb67d] text-white font-bold py-4 px-8 rounded-lg focus:outline-none focus:shadow-outline text-lg transition-all duration-300 cursor-pointer flex items-center justify-center gap-3 transform hover:scale-105 ${loading || !category.trim() ? "opacity-50 cursor-not-allowed transform-none" : ""
                }`}
            >
              {loading ? (
                <>
            ...
            <button
              ...
              disabled={loading || !category.trim()}
              className={`bg-gradient-to-r from-[#7f5af0] to-[#a786df] hover:from-[#a786df] hover:to-[#7f5af0] text-white font-semibold py-4 px-6 rounded-lg focus:outline-none focus:shadow-outline text-lg transition-all duration-300 cursor-pointer flex items-center justify-center gap-2 ${loading || !category.trim() ? "opacity-50 cursor-not-allowed" : ""
                }`}
            >
        ...
        {pdfs.length > 0 && !overallSummary && (
          <div className="bg-[#23272f] shadow-2xl rounded-xl px-10 pt-8 pb-10 mb-8 border-2 border-[#2cb67d]">
            ...
            <button
              ...
              className={`w-full bg-gradient-to-r from-[#2cb67d] to-[#7f5af0] hover:from-[#7f5af0] hover:to-[#2cb67d] text-white font-bold py-4 px-8 rounded-lg focus:outline-none focus:shadow-outline text-lg transition-all duration-300 flex items-center justify-center gap-3 transform hover:scale-105 ${loading || savingToHistory || selectedPdfs.length === 0 ? "opacity-50 cursor-not-allowed transform-none" : "cursor-pointer"
                }`}
            >
        ...
        {overallSummary && (
          <ErrorBoundary>
            <div className="bg-[#23272f] shadow-2xl rounded-xl px-10 pt-8 pb-10 mb-8 border-2 border-[#7f5af0]">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-sm text-[#a786df] bg-[#18181b] px-3 py-1 rounded-lg">
                    {typeof overallSummary === 'object' && overallSummary.total_documents
                      ? `${overallSummary.total_documents} documents`
                      : pdfCount ? `${pdfCount} cases analyzed`
                        : 'Analysis complete'
                    }
                  </div>
                  <button
                    onClick={() => {
                      try {
                        ...
                        if (typeof overallSummary === 'object') {
                          content += `Analysis Type: ${overallSummary.batch_type || overallSummary.analysis_type || 'Legal Analysis'}\n`;
                          content += `Format: ${overallSummary.format || 'structured'}\n`;
                          if (overallSummary.confidence_level) content += `Confidence: ${overallSummary.confidence_level}\n`;
                          content += '\n--- DETAILED ANALYSIS ---\n\n';
                          content += (overallSummary.output_text || overallSummary.summary || 'No detailed analysis available');
                        } else {
                        ...
                        a.href = url;
                        a.download = `${category}-enhanced-analysis-${new Date().toISOString().split('T')[0]}.txt`;
                        a.click();
              ...
              {/* Enhanced Summary Content - Handle both string and object summaries */}
              <div className="bg-[#18181b] border border-[#7f5af0] rounded-lg p-6 mb-6">
                <h4 className="text-lg font-semibold mb-4 text-[#7f5af0]">
                  Enhanced Legal Analysis
                </h4>

                {/* Check if summary is a string (markdown) or object (structured) */}
                {typeof overallSummary === 'string' ? (
                  /* Render markdown summary directly */
                ...
                ) : typeof overallSummary === 'object' && overallSummary !== null ? (
                  /* Handle object summaries */
                  overallSummary.summary ? (
                    /* If object has a 'summary' field with markdown content */
                    <div className="prose prose-invert max-w-none">
                      <ReactMarkdown
                        remarkPlugins={[remarkGfm]}
                        components={{
                    ...
                    /* Use the enhanced renderer for complex objects */
